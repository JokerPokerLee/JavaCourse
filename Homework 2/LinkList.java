...
/*	void destroy()
	...
	void insert(T x, int k)						//insert a new number x after the k-th number
	{
	...
	void insert(T arr[], int n, int k)			//insert an array of n numbers after k-th number
	{
		if (k < 0 || k > size)
			return;
		for (int i = 0; i < n; i++)
	...
	T findByIndex(int k)						//find k-th number
	{
		Node now = head;
		while (k > 0)
		{
	...
	int findByValue(T x)						//find if there exist x
	{
		int res = 1;
		Node now = head.next();
		while (now != null)
	...
	void deleteByValue(T x)						//delete number x
	{
